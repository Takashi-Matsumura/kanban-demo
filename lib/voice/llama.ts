...
/**
 * ローカル LLM（llama.cpp 等の OpenAI 互換サーバ）経路。確信度は返せないため常に null
 * （呼び出し側は confidence: null を「ゲーティングなしで即実行」として扱う）。
 */
export async function interpretWithLlama(
  transcript: string,
  cards: CardCtx[],
  columns: ColumnCtx[],
  ...
  if (scored.length === 0) {
    return { ok: false, error: "対象バッチが特定できません", confidence: null, debug: { llm } };
  }
  ...
  if (!toColumnId) {
    return { ok: false, error: reason, confidence: null, debug: { llm } };
  }
  if (toColumnId === targetCard.columnId) {
    return { ok: false, error: "移動先が現在工程と同じです", confidence: null, debug: { llm } };
  }

  return { ok: true, cardId: targetCard.id, toColumnId, confidence: null, reason, debug: { llm } };
}
